...
export default BaseValidator.create({
    ...
    name(model) {
        ...
        if (validator.empty(name)) {
            model.get('errors').add('name', 'You must specify a name for the category.');
            this.invalidate();
        } else if (name.match(/^,/)) {
            model.get('errors').add('name', 'Category names can\'t start with commas.');
            this.invalidate();
        } else if (!validator.isLength(name, 0, 150)) {
            model.get('errors').add('name', 'Category names cannot be longer than 150 characters.');
            this.invalidate();
        }
    },
